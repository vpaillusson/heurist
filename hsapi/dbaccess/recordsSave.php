...
/**
* Class to add/update/delete Heurist records
*
...
/*
PUBLIC
recordAdd  - creates temporary record for given user
recordSave - saves record
recordDuplicate - duplicates record
recordDelete  - removes records 

recordOwnerAccessCheck  - verifies access right value and is the current user able to change ownership for given record
recordOwnerAccessUpdate - updates ownership and access for set of records
recordGetIncrementedValue - gets new value for given field (move to dbs_???)

PRIVATE
_isWrongAccessRights - checks that access has allowed value
_updateUsrRecPermissions - updates usrRecPermissions table for multigroup view permission
_deleteOneRecord -  deletes given record (along with all dependencies)
_setWorkFlowStage - assigns ownership, visibility to given record header and gets emails for notification to be send
...
class RecordsSave
{
    ...
    */
    public function recordAdd( $record, $return_id_only=false ){
        ...
        if($this->_isWrongAccessRights($access)){
        ...
        if(!$newId){
            ...
            if($access_grps!=null || count($owner_grps)>0){
                $this->_updateUsrRecPermissions($newId, $access_grps, $owner_grps);
            }
    ...
    /**
    * Duplicates given record
    * 
    * @param mixed $rec_ID
    */
    public function recordDuplicate( $rec_ID ){
        ...
        $row = mysql__select_row($mysqli, "SELECT rec_OwnerUGrpID, rec_RecTypeID FROM Records WHERE rec_ID = ".$rec_ID);
        $owner = $row[0];
        $recTypeID = $row[1];
        ...
        while (true) {
            ...
            $new_id = mysql__duplicate_table_record($mysqli, 'Records', 'rec_ID', $rec_ID, null);
            //@todo addRecordIndexEntry(DATABASE, $recTypeID, $rec_ID);
            ...
            $res = mysql__duplicate_table_record($mysqli, 'recDetails', 'dtl_RecID', $rec_ID, $new_id);
            ...
            if(is_array($dty_IDs) && count($dty_IDs)>0){
                foreach($dty_IDs as $dty_ID){            
                    ...
                    $res = $this->recordGetIncrementedValue(array('rtyID'=>$recTypeID, 'dtyID'=>$dty_ID));
                    if($res['status']==HEURIST_OK){
                        ...
                        //.' and dtl_Value='.$rec_ID   //old record id
            ...
            //$fd_res = unregister_for_recid2($rec_ID, $needDeleteFile);
            //if ($fd_res) { $error = "database error - " . $fd_res; break; }

            //@todo update details with new file ids


            $res = mysql__duplicate_table_record($mysqli, 'usrReminders', 'rem_RecID', $rec_ID, $new_id);
            if(!is_int($res)){ $error = $res; break; }

            $res = mysql__duplicate_table_record($mysqli, 'usrRecTagLinks', 'rtl_RecID', $rec_ID, $new_id);
            if(!is_int($res)){ $error = $res; break; }

            $res = mysql__duplicate_table_record($mysqli, 'usrRecPermissions', 'rcp_RecID', $rec_ID, $new_id);
            if(!is_int($res)){ $error = $res; break; }

            //$res = mysql__duplicate_table_record($mysqli, 'recThreadedComments', 'cmt_RecID', $rec_ID, $new_id);
            //if(!is_int($res)){ $error = $res; break; }

            //@todo change all woots with title bookmark: to user:
            /*
            mysql_query('update woots set woot_Title="user:" where woot_Title in (select concat("boomark:",bkm_ID) as title from usrBookmarks where bkm_recID = ' . $rec_ID.')');
            if (mysql_error()) { $error = "database error - " . mysql_error(); break; }
            */

            $res = mysql__duplicate_table_record($mysqli, 'usrBookmarks', 'bkm_RecID', $rec_ID, $new_id);
            if(!is_int($res)){ $error = $res; break; }
            ...
            */
            ...
            $refs_res = mysql__select_list($mysqli, 'recLinks', 'rl_RelationID', 
                '(rl_RelationTypeID is not null) and  (rl_SourceID='.$rec_ID.' or rl_TargetID='.$rec_ID.')');


            foreach ($refs_res as $rel_recid){

                $res = $this->recordDuplicate( $rel_recid );
    ...
    /**
    * Removes heurist records and all dependent entries
    ...
    */
    function recordDelete( $recids, $need_transaction=true, 
        $check_source_links=false, $filterByRectype=0, $progress_session_id=null)
    {
        ...
        if(count($recids)>0){
            ...
            foreach ($recids as $recID) {
                $ownerid = null;
                $access = null;
                $is_allowed = $this->recordOwnerAccessCheck($recID, $ownerid, $access, $rectypes);
            ...
            foreach ($allowed_recids as $recID) {
                ...
                $stat = $this->_deleteOneRecord($recID, $rectypes[$recID]);
    ...
    /**
    * Verifies current user can change ownership and access for given record
    * Beised it assigns valid values to parameters $owner_grps, $access
    *
    * @param mixed $recID
    * @param mixed $owner_grps - array of owners (user groups)
    * @param mixed $access  - access level
    * @param mixed $rectypes - adds reference to record type to this array ($rectypes[$recID] = $rty_ID)
    * @return true if current user can change ownership and access for given record
    */
    public function recordOwnerAccessCheck($recID, &$owner_grps, &$access, &$rectypes)
    {
        ...
        if($access && $this->_isWrongAccessRights($access))
        {
            return false;
        }
    ...
    /**
    * Updates ownership and access for set of records
    * $params
    *     ids - array of record ids
    *     OwnerUGrpID - new ownership
    *     NonOwnerVisibility - access rights
    */
    public function recordOwnerAccessUpdate( $params )
    {
        ...
        if(count($recids)>0){
            ...
            $rectypes = array();//stub param for recordOwnerAccessCheck
            ...
            { 
                ...
                foreach ($recids as $recID) {
                    if(!$this->recordOwnerAccessCheck($recID, $owner_grps, $access, $rectypes)){
                        $noaccess_count++;
                    }else{
                        array_push($allowed_recids, $recID);
                    }
            ...
            while ($k < $cnt_allowed_recids) {
                ...
                if(!$stmt->execute()){
                    ...
                    $this->_updateUsrRecPermissions($chunk, $access_grps, $owner_grps);
                }
    ...
    /**
    * Gets incremeneted value for given field
    * 
    * @param mixed $params
    *               rtyID - record type id
    *               dtyID - base field id  
    */
    public function recordGetIncrementedValue($params){
        ...
        if($rty_ID>0 && $dty_ID>0){

            $mysqli = $system->get_mysqli(); 

            //1. get detail type
            $res = mysql__select_list($mysqli, 'defDetailTypes','dty_Type','dty_ID='.$dty_ID);
            if(is_array($res) && count($res)>0){
                ...
                if($isNumeric){
                    $res = mysql__select_value($mysqli, 'select max(CAST(dtl_Value as SIGNED)) FROM recDetails, Records'
                        .' WHERE dtl_RecID=rec_ID and rec_RecTypeID='.$rty_ID.' and dtl_DetailTypeID='.$dty_ID);    
                }else{
                ...
                return $system->addError(HEURIST_INVALID_REQUEST, 'Get incremented value. Detail type '.$dty_ID.' not found');
            }
        }else{
    ...
    //---------------------------------- PRIVATE METHODS -----------------------
    //
    // Checks that access has allowed value
    //
    private function _isWrongAccessRights( $access ){
        if ($access=='viewable' || $access=='hidden' || $access=='public' || $access=='pending') {
            return false;
        }else{
            $thi->system->addError(HEURIST_INVALID_REQUEST, 'Non-owner visibility value is missing or invalid');
            return true;
        }
    }
    
    //
    // Updates usrRecPermissions table for multigroup view permission
    //
    private function _updateUsrRecPermissions($recIDs, $access_grps, $owner_grps){
    ...
    //
    // Deletes given record (along with all dependencies)
    //
    // It returns
    ...
    //
    function _deleteOneRecord( $id, $rectype ){
        ...
        $links = recordSearchRelated($this->system, array($id), -1, false, 1);
        if($links['status']==HEURIST_OK && count(@$links['data']['reverse'])>0){
        ...
        while(true){
            ...
            if($refs_res){
                while ($row = $refs_res->fetch_assoc()) {
                    $res = $this->_deleteOneRecord( $row['rec_ID'], RT_RELATION);
            ...
            if(is_array($child_records) && count($child_records)>0){
                foreach ($child_records as $recid => $rectypeid) {
                    $res = $this->_deleteOneRecord( $recid, $rectypeid);
                    if( array_key_exists('error', $res) ){
    ...
    /**
    * Assigns ownership, visibility to given record header and gets emails for notification to be send
    * 
    * $new_stage_value - new stage value. It is always >0 and $record['FlagTemporary'] = 0
    * 
    * @param mixed $record - affected record header values
    * @return array( new_value, curr_value, emails )
    */
    private function _setWorkFlowStage( &$record, $new_stage_value, $is_insert){

        $current_value = 0;
        $emails = null;

        if($new_stage_value>0 && @$record['FlagTemporary']!=1){
            ...
            if($current_value!=$new_stage_value){
                ...
                if($rule!=null && 
                    ($rule['swf_StageRestrictedTo']==null 
                        ...
                        || $this->system->is_member($rule['swf_StageRestrictedTo']))
                    ){
                ...
                if($is_allowed){
                    ...
                    $new_stage_value = 0; //not allowed
                }
            }
        }

        return array('new_value'=>$new_stage_value, 'curr_value'=>$current_value, 'emails'=>$emails);
    }
...
function recordSave($system, $record, $use_transaction=true, $suppress_parent_child=false, $update_mode=0){
    ...
    if($record['FlagTemporary']!=1 && $system->defineConstant('DT_WORKFLOW_STAGE')){
        ...
        if($new_swf_stage>0){
            ...
            // and assign $record['swf'] = true, to avoid recordOwnerAccessCheck
            // returns array( new_value, curr_value, emails )
            $swf_res = $this->_setWorkFlowStage($record, $new_swf_stage, $is_insert || $is_save_new_record);
    ...
    if($is_insert){   // ADD NEW RECORD
        ...
        if(!@$record['swf'] && !$this->recordOwnerAccessCheck($recID, $owner_grps, $access, $rectypes)){
            return $system->getError();
        }
        ...
        array_shift($owner_grps); //remove first
        $this->_updateUsrRecPermissions($recID, $access_grps, $owner_grps);
...
* 
*/
function _prepareDetails($rectype, $record, $validation_mode, $recID, $modeImport)
{
    $system = $this->system;
    ...
    */
    ...
    //1. verifies (value, termid, file id, resource id) and prepare details (geo field). verify required field presence
    //2. deletee existing details
    //3. inserts new set
            ...
            switch ($det_types[$dtyID]) {
                ...
                case "relationtype":
                    ...
                    if($validation_mode>1){
                        ...
                        // is given term valid ofr base field ot
                        $isValid = VerifyValue::isValidTerm($term_tocheck, $dtyID);
                ...
                case "resource":
                    ...
                    if($validation_mode>1){
                        ...
                        if($rectype_tocheck){
                            ...
                            $isValid = VerifyValue::isValidPointer( null, $dtyID, $rectype_tocheck[0] );
                            if(!$isValid){
...
//
// Validates WKT value
// returns array of two elements geo type and prepared value
// if WKT is invalid it return array(false, error_message)
//
function prepareGeoValue($mysqli, $dtl_Value){
    ...
    if($geoType=='p'||$geoType=='l'||$geoType=='pl'||$geoType=='c'||$geoType=='r'||$geoType=='m'){
        //geo type is first 2 characters of value
